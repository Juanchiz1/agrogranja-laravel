@extends('layouts.app')
@section('title','Ceba Porcina')
@section('page_title','🏋️ Ceba y Engorde')
...
<div class="section-card" style="text-align:center;padding:30px;">
  ...
  <a href="{{ route('animales.index') }}" class="btn btn-sm btn-primary">Ir a Animales</a>
</div>
@else

@foreach($datosLotes as $animalId => $data)
@php
  $lote       = $data['lote'];
  $pesos      = $data['pesos'];
...
<div class="section-card">
  <div class="section-header">
    <div>
      <div style="font-weight:800;font-size:.95rem;">🐖 {{ $lote->nombre_lote }}</div>
      <div style="font-size:.75rem;color:#64748b;">
        {{ $lote->cantidad }} animales
        @if($lote->raza_porcina) · {{ $lote->raza_porcina }}@endif
        @if($lote->ubicacion) · 📍{{ $lote->ubicacion }}@endif
      </div>
    </div>
    <button onclick="openPesoModal({{ $lote->id }},'{{ addslashes($lote->nombre_lote) }}',{{ $pesos->count()+1 }})"
  ...
  <div style="margin-bottom:12px;">
    <div style="display:flex;justify-content:space-between;font-size:.78rem;margin-bottom:4px;">
      <span style="color:#64748b;">Entrada: {{ $lote->peso_entrada_kg ?? 20 }} kg</span>
      <span style="font-weight:700;color:#ea580c;">{{ $pesoActual }} kg actual</span>
      <span style="color:#64748b;">Meta: {{ $metaKg }} kg</span>
    </div>
    <div class="ceba-avance-wrap">
      <div class="ceba-avance-fill" style="width:{{ $pctAvance }}%;"></div>
    </div>
    <div style="font-size:.72rem;color:#64748b;text-align:center;margin-top:3px;">
      {{ $pctAvance }}% del objetivo
      @if($fechaSacr)
        · Sacrificio proyectado: <strong>{{ \Carbon\Carbon::parse($fechaSacr)->format('d/m/Y') }}</strong>
        ({{ now()->diffInDays($fechaSacr) }} días)
      @endif
    </div>
  </div>
  ...
  <div class="peso-row-porc">
    <span class="peso-semana-porc">Sem {{ $p->semana }}</span>
    <div style="flex:1;margin:0 10px;">
      <div style="background:#e2e8f0;border-radius:4px;height:5px;overflow:hidden;">
        <div style="width:{{ $pctBar }}%;height:100%;border-radius:4px;
                    background:{{ $delta !== null && $delta >= 0 ? '#ea580c' : '#94a3b8' }};"></div>
      </div>
      @if($stdSem)
      <div style="font-size:.67rem;color:#94a3b8;margin-top:1px;">
        Meta {{ $stdSem->peso_meta_kg }} kg
        @if($delta !== null)
          <span class="{{ $delta >= 0 ? 'peso-ok-porc' : 'peso-bajo-porc' }}">
            ({{ $delta >= 0 ? '+' : '' }}{{ $delta }} kg)
          </span>
        @endif
      </div>
    ...
    <div style="text-align:right;">
      <span style="font-weight:800;color:#ea580c;">{{ $p->peso_promedio_kg }} kg</span>
      @if($p->gpd_kg)
      <div style="font-size:.7rem;color:#64748b;">{{ $p->gpd_kg }} kg/día</div>
      @endif
    </div>
  </div>
  @endforeach

  {{-- Gráfica pesos vs tabla --}}
  @if($pesos->count() > 1)
  <div style="position:relative;height:120px;margin-top:10px;">
    <canvas id="chartCeba{{ $lote->id }}"></canvas>
  </div>
  @endif
  @else
  ...
  <div style="margin-top:10px;border-top:1px solid #e2e8f0;padding-top:8px;">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      ...
      <div style="background:#f8fafc;border-radius:8px;padding:6px 10px;text-align:center;">
        <div style="font-size:.65rem;color:#94a3b8;">Sem {{ $c->semana }}</div>
        <div class="{{ ($c->conversion_alimenticia ?? 4) <= 2.8 ? 'ca-val-ok' : (($c->conversion_alimenticia ?? 4) <= 3.2 ? 'ca-val-med' : 'ca-val-bad') }}"
             style="font-size:.9rem;">
          {{ $c->conversion_alimenticia ?? '—' }}
        </div>
      </div>
      @endforeach
      <button onclick="openConvModal({{ $lote->id }},'{{ addslashes($lote->nombre_lote) }}')"
...
<div id="modalPesoCeba" class="modal-overlay" style="display:none;">
  <div class="modal-sheet">
    ...
    <form method="POST" action="{{ route('porcicola.ceba.peso') }}">
      @csrf
...
<div id="modalConvPorcina" class="modal-overlay" style="display:none;">
  <div class="modal-sheet">
    ...
    <form method="POST" action="{{ route('porcicola.ceba.conversion') }}">
      @csrf
...
@endphp
(function(){
  var ctx = document.getElementById('chartCeba{{ $lid }}');
  if (!ctx) return;
  new Chart(ctx, {
    type:'line',
    data:{
      labels:{!! json_encode($sems) !!},
      datasets:[
        { label:'Real', data:{!! json_encode($reales) !!},
          borderColor:'#f97316', backgroundColor:'rgba(249,115,22,.1)',
          borderWidth:2, pointRadius:3, fill:true },
        { label:'Meta ceba comercial', data:{!! json_encode($metas) !!},
