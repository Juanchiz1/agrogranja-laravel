@extends('layouts.app')
@section('title','Cosecha Piscicola')
@section('page_title','Cosecha')
...
<div class="section-card">
  ...
  <div class="cosecha-indicadores">
    <div class="cos-ind" style="background:#f0f9ff;">
      <div class="cos-ind-val" style="color:var(--pisc-azul);">{{ round($totalCosechado,1) }} kg</div>
      <div class="cos-ind-lbl">Total cosechado</div>
    </div>
    <div class="cos-ind" style="background:{{ $caPromedio <= 1.8 ? '#f0fdf4' : '#fffbeb' }};">
      <div class="cos-ind-val" style="color:{{ $caPromedio <= 1.8 ? '#15803d' : '#d97706' }};">
        {{ $caPromedio > 0 ? $caPromedio : 'N/D' }}
      </div>
      <div class="cos-ind-lbl">CA promedio</div>
    </div>
    <div class="cos-ind" style="background:{{ $sobrevPromedio >= 85 ? '#f0fdf4' : '#fffbeb' }};">
      <div class="cos-ind-val" style="color:{{ $sobrevPromedio >= 85 ? '#15803d' : '#d97706' }};">
        {{ $sobrevPromedio > 0 ? $sobrevPromedio.'%' : 'N/D' }}
      </div>
...
<div class="section-card">
  ...
  <div style="padding:10px 0;border-bottom:1px solid #e2e8f0;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;">
      <div>
        ...
        <div style="font-size:.75rem;color:#64748b;">
          {{ $s->especie_cultivada }} · Dia {{ $diasCult }} de cultivo
          @if($s->area_m2) · {{ $s->area_m2 }} m²@endif
        </div>
        <div style="font-size:.75rem;color:#64748b;margin-top:2px;">
          Biomasa: {{ $biomasaAct }} kg · Peso prom: {{ $pesoPromG }} g
          · Alimento total: {{ $aliAcum }} kg
        </div>
      </div>
      <div>
        @if($listaParaCosechar)
    ...
    <button onclick="openCosechaModal({{ $s->id }},{{ $s->estanque_id }},'{{ addslashes($s->nombre_estanque) }}',{{ $s->cantidad_actual ?? $s->cantidad_alevinos }},{{ $aliAcum }})"
...
<div class="section-card" style="text-align:center;padding:20px;color:#64748b;">
  ...
  <br><a href="{{ route('piscicola.siembra') }}" style="color:var(--pisc-azul);">Ver siembras</a>
</div>
@endif
...
<div class="section-card">
  ...
  <div style="border:1.5px solid #e2e8f0;border-radius:10px;padding:10px 12px;margin-bottom:8px;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;">
      <div>
        <div style="font-weight:700;font-size:.9rem;">{{ $c->nombre_estanque }}</div>
        <div style="font-size:.74rem;color:#64748b;">{{ $cFecha }} · {{ $c->dias_cultivo ?? '—' }} dias</div>
      </div>
      <div style="text-align:right;">
        <div style="font-size:1.1rem;font-weight:800;color:var(--pisc-azul);">{{ $c->biomasa_cosechada_kg }} kg</div>
        @if($c->valor_total_cop)
        <div style="font-size:.72rem;color:#15803d;font-weight:700;">
          ${{ number_format($c->valor_total_cop, 0, ',', '.') }}
        </div>
        @endif
      </div>
    </div>
    <div class="cosecha-indicadores" style="margin-top:8px;">
      <div class="cos-ind" style="background:#f8fafc;">
        <div class="cos-ind-val" style="color:{{ $caColor }};font-size:.9rem;">
          {{ $cCA ?? 'N/D' }}
        </div>
        <div class="cos-ind-lbl">CA</div>
      </div>
      <div class="cos-ind" style="background:#f8fafc;">
        <div class="cos-ind-val" style="color:{{ ($cSobr ?? 0) >= 85 ? '#15803d' : '#d97706' }};font-size:.9rem;">
          {{ $cSobr !== null ? $cSobr.'%' : 'N/D' }}
        </div>
        <div class="cos-ind-lbl">Sobrevivencia</div>
      </div>
      <div class="cos-ind" style="background:#f8fafc;">
        <div class="cos-ind-val" style="font-size:.9rem;">
          {{ $cRend !== null ? $cRend.' kg/m²' : 'N/D' }}
        </div>
        <div class="cos-ind-lbl">Rendimiento</div>
      </div>
    </div>
    @if($c->peso_promedio_final_g)
    <div style="font-size:.72rem;color:#64748b;margin-top:4px;">
      Peso final prom: {{ $c->peso_promedio_final_g }} g ·
      {{ $c->cantidad_cosechada }} peces cosechados
      @if($c->comprador) · Comprador: {{ $c->comprador }}@endif
    </div>
    @endif
  </div>
  @endforeach
...
<div id="modalCosecha" class="modal-overlay" style="display:none;">
  <div class="modal-sheet">
    ...
    <form method="POST" action="{{ route('piscicola.cosecha.store') }}">
      @csrf
