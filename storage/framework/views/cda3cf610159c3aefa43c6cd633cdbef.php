...
<div class="tabs-rep">
  ...
  <a href="?tab=lineas&anio=<?php echo e($anio); ?>" class="tab-rep <?php echo e($tab==='lineas'?'active':''); ?>">&#127806; Lineas</a>
...
<?php if(!isset($rentabilidadLineas) || count($rentabilidadLineas) === 0): ?>
<div class="empty-state"><div class="emoji">&#128075;</div><p>Sin datos de lineas para <?php echo e($anio); ?>. Verifica que el ReporteController incluya las variables de Fase 9.</p></div>
<?php else: ?>
<div class="stats-row" style="margin-bottom:14px;">
  <div class="stat-mini" style="background:var(--verde-bg)"><div class="stat-mini-val text-green">$<?php echo e(number_format($totalIngresos/1000,1)); ?>k</div><div class="stat-mini-label">Ingresos</div></div>
  <div class="stat-mini" style="background:var(--marron-bg)"><div class="stat-mini-val" style="color:var(--marron)">$<?php echo e(number_format($totalGastos/1000,1)); ?>k</div><div class="stat-mini-label">Gastos</div></div>
  <div class="stat-mini" style="background:<?php echo e($balance>=0?'var(--verde-bg)':'#fef2f2'); ?>"><div class="stat-mini-val" style="color:<?php echo e($balance>=0?'var(--verde-dark)':'var(--rojo)'); ?>"><?php echo e($balance>=0?'+':''); ?>$<?php echo e(number_format(abs($balance)/1000,1)); ?>k</div><div class="stat-mini-label">Balance</div></div>
</div>

<div class="rep-card">
  <div class="rep-card-title">Rentabilidad por linea productiva</div>
  <?php $rlSafe2 = isset($rentabilidadLineas) ? $rentabilidadLineas : []; $maxAbsRent = $rlSafe2 ? max(1, max(array_map(fn($r) => abs($r['rentabilidad']), $rlSafe2))) : 1; ?>
  <?php $__currentLoopData = ($rentabilidadLineas ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lk => $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
  <?php
  ...
  <div style="margin-bottom:14px;padding-bottom:14px;border-bottom:1px solid var(--border);">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
      <div>
        <span style="font-size:.95rem;font-weight:700;"><?php echo $r['emoji']; ?> <?php echo e($r['nombre']); ?></span>
        <span style="font-size:.72rem;color:#64748b;margin-left:8px;">Margen: <strong style="color:<?php echo e($margenC); ?>"><?php echo e($r['margen']); ?>%</strong></span>
      </div>
      <div style="text-align:right;">
        <div style="font-size:.92rem;font-weight:800;color:<?php echo e($colorB); ?>;"><?php echo e($r['es_rentable'] ? '+' : ''); ?>$<?php echo e(number_format($r['rentabilidad']/1000,1)); ?>k</div>
    ...
    <div style="background:#e2e8f0;border-radius:6px;height:8px;overflow:hidden;margin-bottom:4px;">
      <div style="width:<?php echo e($pctBarra); ?>%;height:100%;border-radius:6px;background:<?php echo e($colorB); ?>;"></div>
    </div>
    <div style="display:flex;justify-content:space-between;font-size:.72rem;color:#64748b;">
      <span style="color:var(--verde-dark);">Ingresos: $<?php echo e(number_format($r['ingresos']/1000,1)); ?>k</span>
      <span style="color:var(--rojo);">Gastos: $<?php echo e(number_format($r['gastos']/1000,1)); ?>k</span>
    </div>
  </div>
  <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
</div>
...
<?php if(!empty($kpisLineasReporte ?? [])): ?>
<div class="rep-card">
  <div class="rep-card-title">KPIs especificos por linea</div>
  <?php $__currentLoopData = ($kpisLineasReporte ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lk => $k): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
  <div style="margin-bottom:14px;">
    <div style="font-weight:700;font-size:.88rem;margin-bottom:8px;"><?php echo e($k['emoji']); ?> <?php echo e($k['titulo']); ?></div>
    ...
    <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #f1f5f9;font-size:.82rem;">
      <div>
        ...
        <?php if(isset($item['meta'])): ?><div style="font-size:.68rem;color:#94a3b8;">Meta: <?php echo e($item['meta']); ?></div><?php endif; ?>
      </div>
      <span style="font-weight:800;color:#1e293b;margin-left:8px;white-space:nowrap;"><?php echo e($item['valor']); ?></span>
    </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  </div>
  <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
?>
if(document.getElementById('chartLineasComp') && <?php echo json_encode(count($rlSafe), 15, 512) ?> > 0) {
  new Chart(document.getElementById('chartLineasComp'), {
    type:'bar',
    data:{ labels:<?php echo json_encode($lineasLabels, 15, 512) ?>, datasets:[
      { label:'Ingresos',          data:<?php echo json_encode($lineasIng, 15, 512) ?>,  backgroundColor:'rgba(61,139,61,.75)', borderRadius:4 },
      { label:'Gastos',            data:<?php echo json_encode($lineasGast, 15, 512) ?>, backgroundColor:'rgba(122,79,42,.65)', borderRadius:4 },
      { label:'Rentabilidad neta', data:<?php echo json_encode($lineasRent, 15, 512) ?>, backgroundColor:'rgba(37,99,235,.6)',  borderRadius:4 }
    ]},
    options:{ responsive:true, plugins:{legend:{position:'bottom'}},
...
<div class="section-card" style="background:var(--surface);border-radius:var(--radius-lg);padding:16px;margin-top:16px;box-shadow:var(--shadow-sm);border-left:3px solid var(--verde-dark);">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;">
        ...
        <form method="POST" action="<?php echo e(route('metricas.snapshot')); ?>" style="flex:1;min-width:140px;">
            <?php echo csrf_field(); ?>
            ...
            <button type="submit" class="btn btn-primary btn-full" style="font-size:.83rem;">
                📤 Enviar métricas — <?php echo e(\Carbon\Carbon::now()->subMonth()->translatedFormat('F Y')); ?>

            </button>
        </form>
 
        
        <form method="POST" action="<?php echo e(route('metricas.snapshot')); ?>" style="flex:1;min-width:140px;display:flex;gap:6px;">
            <?php echo csrf_field(); ?>
            <input type="month" name="periodo" class="form-control" value="<?php echo e(\Carbon\Carbon::now()->subMonth()->format('Y-m')); ?>" style="flex:1;">
